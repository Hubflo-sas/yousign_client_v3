...
module YousignClientV3
  class ConsumptionApi
    ...
    def get_consumption_addon_with_http_info(opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'addons'] && !opts[:'addons'].all? { |item| ['electronic_signature', 'advanced_electronic_signature', 'qualified_electronic_signature_identity_verification', 'qualified_electronic_signature_saved_identity', 'electronic_seal', 'advanced_electronic_seal', 'archiving'].include?(item) }
        fail ArgumentError, 'invalid value for "addons", must include one of electronic_signature, advanced_electronic_signature, qualified_electronic_signature_identity_verification, qualified_electronic_signature_saved_identity, electronic_seal, advanced_electronic_seal, archiving'
      end
      # resource path
      ...
      return_type = opts[:return_type] || 'GetConsumptionAddon200Response' 
    ...
    def get_consumption_detail_with_http_info(from, to, opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'breakdown_type'] && !['organization', 'workspace'].include?(opts[:'breakdown_type'])
        fail ArgumentError, 'invalid value for "breakdown_type", must be one of organization, workspace'
      end
      # resource path
      ...
      return_type = opts[:return_type] || 'GetConsumptionDetail200Response' 
    ...
    def get_consumptions_with_http_info(from, to, opts = {})
      ...
      return_type = opts[:return_type] || 'Consumption' 
